...
class CapacitiveButtons(object):
    ...
    def reset_interrupt(self):
        ...
        logging.debug("Will reset interrupt. MAIN: %s", bin(self.read_register(CapacitiveButtons.MAIN)))
        self.write_register(
            CapacitiveButtons.MAIN,
            self.read_register(CapacitiveButtons.MAIN)
                & ~CapacitiveButtons.MAIN_INT
        )
        logging.debug("Did reset interupt. MAIN is  %s", bin(self.read_register(CapacitiveButtons.MAIN)))
        
    @property
    def driver_name(self):
        return "CapacitiveButtons"

    @property
    def product_id(self):
        return self.read_register(CapacitiveButtons.PRODID)

    @property
    def manufacturer_id(self):
        return self.read_register(CapacitiveButtons.MANUID)

    @property
    def revision(self):
        return self.read_register(CapacitiveButtons.REV)
    ...
    def repeat_enabled_status(self):
        return self.read_register(CapacitiveButtons.REPEAT_ENABLE_REG)
    ...
    def repeat_enabled_status(self, status):
        ...
        self.write_register(
            CapacitiveButtons.REPEAT_ENABLE_REG,
            CapacitiveButtons.REPEAT_ENABLE_ALL & status
        )
    ...
    def cfg2(self):
        ...
        return self.read_register(CapacitiveButtons.CFG2_REG)
    ...
    def cfg2(self, value):
        ...
        self.write_register(
            CapacitiveButtons.CFG2_REG,
            value & CapacitiveButtons.CFG2_ENABLE_ALL
        )
    ...
    def calibrate(self, pins = CAL_ACT_ALL):
        ...
        self.write_register(
            CapacitiveButtons.CAL_ACT_REG,
            pins & CapacitiveButtons.CAL_ACT_ALL
        )
    ...
    def multitouch_enabled(self):
        ...
        return self.read_register(CapacitiveButtons.MTBLK) == 0
    ...
    def multitouch_enabled(self, enabled):
        ...
        if enabled is True:
            self.write_register(CapacitiveButtons.MTBLK, 0)
        else:
            # TODO verify this
            self.write_register(CapacitiveButtons.MTBLK, 1)
    ...
    def leds_linked(self):
        ...
        return self.read_register(CapacitiveButtons.LEDLINK) == 0xFF
    ...
    def leds_linked(self, linked):
        ...
        if linked is True:
            self.write_register(CapacitiveButtons.LEDLINK, 0xFF)
        else:
            # TODO verify this
            self.write_register(CapacitiveButtons.LEDLINK, 0)

    @property
    def touched(self):
        touchval = self.read_register(CapacitiveButtons.SENINPUTSTATUS)
        result = list()
        if touchval > 0:
            for i in range(8):
                if touchval & 1 << i:
                    result.append(i + self._touch_offset)
...
if __name__ == "__main__":
    ...
    #cap1 = CapacitiveButtons(cap1_addr, bus)
    cap2 = CapacitiveButtons(cap2_addr, bus, touch_offset = 0)
    ...
    cap2.leds_linked = False

    # Speed it up
    #cap1.write_register(CapacitiveButtons.STANDBYCFG, 0x30)
    cap2.write_register(CapacitiveButtons.STANDBYCFG, 0x30)

    #print cap1
    last_touched = cap2.touched
    while (1):
        touched = list()
        touched = cap2.touched
        #print (touched)
        #if (touched):
        #    for i in range(8):
        #        pin_bit = 1 << i
